...
| UI | All six primary screens remain functional over one centralized palette-aware visual foundation; semantic properties distinguish typography, surfaces, action hierarchy, text badges, persistent messages, focus, disabled state, and the restrained green accent while retaining the native font, title bar, dialogs, and system theme; New Prediction and Forecast Review are the representative M39 applications; Predictions retains all completed v0.5 retrieval behavior; selected navigation and action icons remain local, palette-aware Lucide SVGs rendered through QtSvg while visible text and accessible names remain authoritative |
...
| Automated tests | Complete v0.1-v0.5 coverage plus focused M39 tests for shared tokens, contrast-safe light/dark semantic colors, role assignment, relative native-font typography, focus and disabled selectors, accessible action/message names, Qt animation preference, palette-driven stylesheet and Lucide refresh, representative page/dialog integration, and the visual boundary's independence from domain, application, analytics, and data modules |

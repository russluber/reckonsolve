...
Status: v0.3 source release complete; v0.4 implemented through Milestone 27
...
Milestone 27 exposes M26's historically complete terminal-history operations through focused desktop dialogs. Binary and Numeric Prediction Detail now derives the displayed Resolution or Invalid reason from the complete correction chain, retains the original terminal time and scoring forecast, and reveals the original plus every correction in a collapsed audit section. Every correction is deliberately confirmed; outcome and exact actual-value changes receive a score-impact warning and require an explanation, while notes, later Postmortems, and Invalid-reason corrections remain lightweight. Dialogs carry the latest correction identifier so cancellation, no-op, stale, conflicting, and wrong-lifecycle attempts append nothing. M25's complete v0.3 CLI and every earlier desktop workflow remain intact; CLI read parity remains M31.
...
| Domain and application operations | Complete v0.3 workflows plus M26 terminal-history reads and atomic correction/completion operations; M27 exposes Resolution and Invalid-reason correction while Postmortem completion remains UI-hidden until M30 |
...
| Automated tests | Complete v0.1-v0.3 coverage plus M26 persistence/domain coverage and M27 real-database Qt coverage for confirmation, cancellation, no-op, required explanation, exact Numeric corrections, later Postmortems, audit rendering, corrected analytics, restart, and stale-dialog rejection |

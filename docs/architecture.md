...
Status: Milestone 24 v0.3 terminal lifecycle and guarded deletion complete
...
Milestone 24 completes the CLI's forecast lifecycle with type-aware `resolve`, `invalidate`, and guarded `delete` commands without a schema change or production dependency. Each terminal command displays the current scoring forecast and irreversible consequence, collects type-appropriate values and optional one-line text, and requires explicit confirmation before invoking an existing application operation. Exact scoring-revision capture, Invalid exclusion, untouched-Open deletion eligibility, deadline-derived Locked behavior, and stale-context rejection therefore remain shared with the desktop interface. The complete M21 through M23 CLI behavior and v0.2 persistence, transfer, analytics, UI, and private-build behavior remain intact.
...
| Entry points | `reckonsolve` and `python -m reckonsolve` use the stable GUI identity; `reckonsolve-dev` uses the development GUI identity; `reckonsolve-cli` and `reckonsolve-cli-dev` provide paired M21 reads, M22 creation, M23 active-forecast mutations, and M24 terminal lifecycle; the private frozen entry adds only its disposable build-smoke path |
...
| Domain and application operations | Complete Binary and Numeric creation, revision, Journal, Review, terminal lifecycle, type-aware Dashboard/archive/analytics queries, guarded deletion, backup, and relational CSV export; CLI reads and active mutations reuse these operations without direct SQL |
...
| Automated tests | Complete v0.1/v0.2 coverage plus paired CLI identity/read coverage, M22 creation, M23 active forecasting, and M24 terminal confirmation, exactness, scoring selection, invalidation, deletion, deadline, concurrency, lock, and restart integration tests |

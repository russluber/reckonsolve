...
Status: Milestone 23 v0.3 active-forecast CLI mutations complete
...
Milestone 23 extends the v0.3 command-line companion with type-aware `revise`, `journal`, and `review` commands without a schema change or production dependency. Each command displays the current forecast, retains the reviewed revision and metadata tokens through its prompts, and invokes exactly one existing application operation. Binary and Numeric immutable history, lifecycle and deadline eligibility, Journal anchors, Forecast Review freshness, and stale-context rejection therefore remain shared with the desktop interface. The complete M21 read model, M22 creation workflows, and v0.2 persistence, transfer, analytics, UI, and private-build behavior remain intact.
...
| Entry points | `reckonsolve` and `python -m reckonsolve` use the stable GUI identity; `reckonsolve-dev` uses the development GUI identity; `reckonsolve-cli` and `reckonsolve-cli-dev` provide paired M21 reads, M22 creation, and M23 active-forecast mutations; the private frozen entry adds only its disposable build-smoke path |
...
| Automated tests | Complete v0.1/v0.2 coverage plus paired CLI identity/read coverage, M22 creation, and M23 type-aware revision, Journal, Review, cancellation, freshness, lifecycle, and concurrency integration tests |
...
src/reckonsolve/
  ...
  cli_mutations.py     M23 revision, Journal, and Review prompt orchestration
...
The approved v0.3 plan adds a human-directed CLI companion through Milestones 21 through 25. M21 implements the shared-data foundation and read model: paired source entry points, identity-selected database composition, `--help`, `--version`, filtered `list`, and complete type-aware `show`. M22 adds interactive `create binary` and `create numeric` flows that preserve existing defaults, exactness, optional details, atomicity, and cancellation behavior. M23 adds type-aware `revise`, `journal`, and `review`, retaining existing immutable-history, forecast-anchor, freshness, lifecycle, deadline, and optimistic-concurrency semantics. All three milestones use the existing version-12 schema and application operations, so records remain one canonical history visible through either matching interface.

Terminal lifecycle, CLI backup/export, and the corresponding cross-process hardening remain owned by M24 and M25. The CLI is currently source-distributed through `uv`; a separately frozen executable, installer integration, noninteractive scripting API, terminal analytics, live inter-process refresh, and logo work remain outside v0.3.

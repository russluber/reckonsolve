...
| Automated tests | Complete v0.1-v0.5 coverage plus focused M39 visual-system coverage, M40 shell tests for hierarchy, active routes, compact-mode accessibility and persistence, contextual return without archive refresh, keyboard operation, identity isolation, and safe window-state recovery, M41 coverage for shared page semantics, Dashboard/Settings presentation, persistent result/error separation, notification coalescing, modal deferral, navigation, dismissal, and failure isolation, M42 coverage for type-aware creation guidance, Detail hierarchy and action roles, selectable history, and shared dialog roles, M42A coverage for Numeric metadata atomicity and dialog behavior, and M43 coverage for stable control grouping, minimum-size scrolling, useful tag/date dimensions, structured result semantics, direct mouse/keyboard activation, and styled tag-management status/actions |

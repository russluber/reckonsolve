...
Public Binary and Numeric creation deliberately remains on the legacy contracts during M46. The presence of prospective schema rows and domain constructors is not a partial product switch and does not authorize a UI, CLI, scoring, or export path to guess new-model behavior. M47 will introduce the first complete active Binary trajectory workflow; later milestones will add its terminal scoring path, the complete five-quantile Numeric workflow, separate analytics, and format-version-four portability in the order fixed by the product specification.

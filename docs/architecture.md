...
| UI | All six primary screens are functional; Predictions preserves its blank-query archive and provides responsive explainable full-text results with safe emphasis and source-aware Detail navigation, multi-tag matching, derived attention, inclusive date-range, deterministic-sort controls, and named dynamic Saved Views with explicit update/rename/delete state; selected navigation and action icons are local, palette-aware Lucide SVGs rendered through QtSvg while visible text and accessible names remain authoritative |
...
| Domain and application operations | Complete v0.4 behavior plus safe search parsing and snippets, explainable per-source fragments, deterministic grouped ranking, All/Any matching, corpus spelling guidance, effective/history scope, one shared status/type/multi-tag/attention/date/sort archive query, mutable Saved View configuration operations, presentation-ready forecast and effective terminal summaries, and explicit search check/rebuild operations |

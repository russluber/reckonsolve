...
Status: v0.6 source release implemented through Milestone 45
Last reviewed: 2026-09-05

This document describes how Reckonsolve is structured from the completed binary v0.1 baseline through the completed v0.2, v0.3, v0.4, v0.5, and v0.6 source releases. The [product specification](product-spec.md) governs product behavior, scope, terminology, and acceptance criteria. This document translates those requirements into technical boundaries without replacing them.

## 1. Current implementation

Milestones 26 through 31 complete the v0.4 terminal-correction, scorecard, update-feedback, Postmortem, CLI-read, portability, and private-build work. Milestones 32 through 38 complete v0.5's presentation-neutral full-text retrieval, rich archive query, dynamic Saved Views, transactional tag maintenance, CLI parity, and release hardening on schema version 15. Milestone 39 begins v0.6 without a migration: `ui/visual_system.py` resolves semantic colors from the active Qt palette, builds one inherited stylesheet, defines shared spacing/radius/motion and relative native-font roles, and assigns reusable action, surface, badge, and persistent-message properties. `MainWindow` refreshes that boundary and the existing local Lucide icons after palette changes. New Prediction and Forecast Review are the representative page and dialog; their operations, signals, layout sequence, persistence, and validation remain unchanged, while the earlier search-match cue now uses the centralized semantic selector rather than a widget-local stylesheet. Milestone 40 restructures only the application shell: New Prediction is a prominent action, Dashboard/Predictions/Analytics are the permanent primary destinations, Settings is a bottom utility, and Prediction Detail is contextual with an explicit return path. Expanded or compact sidebar mode and safe normal-window geometry/maximized state live in a separate identity-scoped INI file beside the database, never in canonical SQLite. Milestone 41 adds small reusable page-header, content-panel, badge, empty-state, and persistent-message widgets, applies them to Dashboard and Settings, and places routine success acknowledgment in a single shell-level overlay. The notification host is presentation-only, owns no operation, catches its own display failures, waits behind modal decisions, and cannot roll back or disguise the canonical result that was committed before its signal was emitted. Milestone 42 reuses those primitives across creation, Binary and Numeric Detail, and all focused dialogs. It changes widget composition and semantic roles only: existing application operations remain the sole owners of validation, concurrency checks, history, lifecycle transitions, and persistence. Milestone 42A makes the pre-existing metadata operation type-neutral and reuses its dialog from Numeric Detail; no type-specific SQL, schema change, or duplicate definition-history rule is introduced. Milestone 43 restyles the existing Predictions and tag-management presentation around the unchanged v0.5 application queries and mutations. Milestone 44 completes the broad presentation rollout in Analytics, installs guarded navigation-only shortcuts, documents them in Settings and shell tooltips, and strengthens focus order and accessible descriptions without changing an analytical or canonical fact. Milestone 45 closes v0.6 with complete regression and portability evidence, a disposable visual-review matrix, versioned source-release documentation, and a relocated frozen smoke that treats all schema-version-15 rows as unchanged across presentation-only use and GUI restart.
...
| Automated tests | Complete v0.1-v0.5 coverage plus focused M39 visual-system coverage, M40 shell tests for hierarchy, active routes, compact-mode accessibility and persistence, contextual return without archive refresh, keyboard operation, identity isolation, and safe window-state recovery, M41 coverage for shared page semantics, Dashboard/Settings presentation, persistent result/error separation, notification coalescing, modal deferral, navigation, dismissal, and failure isolation, M42 coverage for type-aware creation guidance, Detail hierarchy and action roles, selectable history, and shared dialog roles, M42A coverage for Numeric metadata atomicity and dialog behavior, M43 coverage for stable control grouping, minimum-size scrolling, useful tag/date dimensions, structured result semantics, direct mouse/keyboard activation, and styled tag-management status/actions, M44 coverage for Analytics composition, responsive summaries, shortcut identity/guarding, documentation, focus order, and accessible analytical alternatives, and M45 all-table compatibility evidence proving presentation-only use preserves every schema-version-15 canonical and derived row |

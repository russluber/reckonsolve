...
| Automated tests | Complete v0.1-v0.4 coverage plus M32 parser/ranking, source projection, migration, atomic refresh/rollback, restart, independent-connection, isolation, corruption-check, and rebuild coverage; M33 grouped-result, safe-snippet, desktop-control, retained-failure, and contextual-navigation coverage; M34 shared-query, combined filter, terminal-correction date, null-sort, and desktop-control coverage; M35 configuration persistence, stable tag references, dynamic re-query, duplicate names, restart, backup, isolation, migration rollback, and desktop workflow coverage; M36 retained-tag listing, display rename, merge/delete deduplication, search refresh, stale-context, rollback, restart, backup, independent-connection, and confirmation/cancellation coverage; and M37 CLI help, grouped result formatting, source snippets, Any-word fallback, superseded history, rich filters, Saved View listing/execution, error handling, and read-only persistence coverage |

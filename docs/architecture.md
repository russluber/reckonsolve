...
Status: v0.3 source release complete; v0.4 implemented through Milestone 28
...
Milestones 26 and 27 provide historically complete terminal-history operations and focused desktop correction dialogs. Binary and Numeric Prediction Detail derives the displayed Resolution or Invalid reason from the complete correction chain, retains the original terminal time and scoring forecast, and reveals the original plus every correction in a collapsed audit section. Milestone 28 adds a separate, type-aware scorecard section on Resolved Detail: it derives individual metrics from the immutable captured scoring revision and latest effective outcome or exact actual value. A score-affecting correction recomputes that one card and shows a correction notice; it never creates another scoring observation or changes the original resolution position. M25's complete v0.3 CLI and every earlier desktop workflow remain intact; CLI read parity remains M31.
...
| Entry points | `reckonsolve` and `python -m reckonsolve` use the stable GUI identity; `reckonsolve-dev` uses the development GUI identity; `reckonsolve-cli`/`rsc` and `reckonsolve-cli-dev`/`rscd` provide the matching complete M21-M25 source CLI; the private frozen entry adds only its disposable build-smoke path |
...
| Domain and application operations | Complete v0.3 workflows plus M26 terminal-history reads and atomic correction/completion operations; M27 exposes Resolution and Invalid-reason correction while Postmortem completion remains UI-hidden until M30; M28 adds a read-only per-Prediction scorecard operation |
| Analytics | Exactly-once type-aware scoring selection uses the latest effective corrected outcome with the original Resolution time and captured scoring revision; pure Binary and Numeric scorecard projections reuse those observations and remain unit-safe |
| Automated tests | Complete v0.1-v0.3 coverage plus M26 persistence/domain coverage, M27 real-database Qt coverage for correction behavior, and M28 pure, operation, and Qt coverage for scorecards, correction recomputation, Numeric endpoint containment, and excluded records |

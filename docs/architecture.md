...
Status: v0.4 source release complete; v0.5 implementation through Milestone 32
Last reviewed: 2026-08-27

This document describes how Reckonsolve is structured from the completed binary v0.1 baseline through the completed v0.2, v0.3, and v0.4 source releases and the implemented Milestone 32 foundation for v0.5. The [product specification](product-spec.md) governs product behavior, scope, terminology, and acceptance criteria. This document translates those requirements into technical boundaries without replacing them.

## 1. Current implementation

Milestones 26 through 31 complete the v0.4 terminal-correction, scorecard, update-feedback, Postmortem, CLI-read, portability, and private-build work. Milestone 32 begins v0.5 with a presentation-neutral full-text-search foundation: schema version 14 adds a rebuildable SQLite FTS5 projection; ordinary query parsing and pure deterministic ranking return grouped Prediction hits with exact source provenance; and every searchable canonical write refreshes affected documents before its existing transaction commits. No M32 desktop or CLI search surface is exposed yet.
...
| Domain and application operations | Complete v0.4 behavior plus safe search parsing, explainable per-source fragments, deterministic grouped ranking, All/Any matching, corpus spelling guidance, effective/history scope, and explicit search check/rebuild operations |
...
| Automated tests | Complete v0.1-v0.4 coverage plus M32 parser/ranking, source projection, migration, atomic refresh/rollback, restart, independent-connection, isolation, corruption-check, and rebuild coverage |
...
`domain/search.py` parses ordinary words and fragment-local quoted phrases without exposing raw FTS syntax, provides incremental final-token prefix matching, normalizes case and common diacritics, and applies pure deterministic grouping and source-aware ranking. `data/search.py` issues parameterized FTS queries, preserves current-Question literal substring behavior, and derives conservative single-edit suggestions from the user's own corpus. The application operation reports rather than silently applies an available Any-word fallback and converts repair failures into an explicit expected error. M32 intentionally exposes none of this through desktop widgets or CLI commands; M33 owns the first user-facing surface.
...
M32 deliberately stops below presentation. The Predictions-screen search and unified structured filters remain M33; saved views, review workflow, analytics slice breakdowns, and release hardening remain their separately authorized v0.5 milestones. Those layers must reuse this query and provenance contract rather than issue widget SQL or establish a second index.

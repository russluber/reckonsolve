...
Status: v0.4 source release complete; v0.5 implementation through Milestone 36
Last reviewed: 2026-08-28

This document describes how Reckonsolve is structured from the completed binary v0.1 baseline through the completed v0.2, v0.3, and v0.4 source releases and the implemented Milestones 32 through 36 search foundation, desktop surface, archive-query controls, dynamic Saved Views, and transactional tag-library management for v0.5. The [product specification](product-spec.md) governs product behavior, scope, terminology, and acceptance criteria. This document translates those requirements into technical boundaries without replacing them.

## 1. Current implementation

Milestones 26 through 31 complete the v0.4 terminal-correction, scorecard, update-feedback, Postmortem, CLI-read, portability, and private-build work. Milestone 32 begins v0.5 with a presentation-neutral full-text-search foundation: schema version 14 adds a rebuildable SQLite FTS5 projection; ordinary query parsing and pure deterministic ranking return grouped Prediction hits with exact source provenance; and every searchable canonical write refreshes affected documents before its existing transaction commits. Milestone 33 exposes that shared query on the Predictions screen with explainable grouped results, safe snippets, current-versus-superseded scope, deliberate All/Any and spelling fallbacks, and type-aware navigation to the matching current Detail context. Milestone 34 adds one shared, presentation-neutral archive query that applies multiple-tag All/Any, type, status, derived attention, local-calendar date ranges, and deterministic sorts before rendering or final relevance ordering. Milestone 35 adds schema version 15 and named mutable Saved Views: a normalized display name and complete dynamic archive configuration, with selected tags retained by stable tag identifier rather than copied display text or result membership. The desktop replays that configuration through the existing archive query and exposes explicit update, rename, and deletion workflows. Milestone 36 uses those stable identities and existing search-dirty triggers without another migration: a secondary Manage Tags workflow previews and atomically applies global rename, many-to-one merge, and deletion across Prediction and Saved View references while advancing affected metadata contexts. CLI search and Saved View execution remain deferred to Milestone 37.
...
| UI | All six primary screens are functional; Predictions preserves its blank-query archive and provides responsive explainable full-text results with safe emphasis and source-aware Detail navigation, multi-tag matching, derived attention, inclusive date-range, deterministic-sort controls, named dynamic Saved Views with explicit update/rename/delete state, and a secondary filtered Manage Tags dialog with association counts and confirmed global actions; selected navigation and action icons are local, palette-aware Lucide SVGs rendered through QtSvg while visible text and accessible names remain authoritative |
...
| Domain and application operations | Complete v0.4 behavior plus safe search parsing and snippets, explainable per-source fragments, deterministic grouped ranking, All/Any matching, corpus spelling guidance, effective/history scope, one shared status/type/multi-tag/attention/date/sort archive query, mutable Saved View configuration operations, previewed transactional tag-library operations, presentation-ready forecast and effective terminal summaries, and explicit search check/rebuild operations |
...
| Automated tests | Complete v0.1-v0.4 coverage plus M32 parser/ranking, source projection, migration, atomic refresh/rollback, restart, independent-connection, isolation, corruption-check, and rebuild coverage; M33 grouped-result, safe-snippet, desktop-control, retained-failure, and contextual-navigation coverage; M34 shared-query, combined filter, terminal-correction date, null-sort, and desktop-control coverage; M35 configuration persistence, stable tag references, dynamic re-query, duplicate names, restart, backup, isolation, migration rollback, and desktop workflow coverage; and M36 retained-tag listing, display rename, merge/delete deduplication, search refresh, stale-context, rollback, restart, backup, independent-connection, and confirmation/cancellation coverage |

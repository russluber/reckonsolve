...
Status: v0.3 source release complete; v0.4 implemented through Milestone 30
...
Milestones 26 and 27 provide historically complete terminal-history operations and focused desktop correction dialogs. Binary and Numeric Prediction Detail derives the displayed Resolution or Invalid reason from the complete correction chain, retains the original terminal time and scoring forecast, and reveals the original plus every correction in a collapsed audit section. Milestone 28 adds type-aware resolved-prediction scorecards. Milestone 29 extends the existing Analytics screen with retrospective initial-versus-final feedback. Milestone 30 adds the calm, Resolved-only Needs Postmortem queue and a confirmed append-only Skip action; its completion remains visible on Prediction Detail even if a Postmortem is added later. M25's complete v0.3 CLI and every earlier desktop workflow remain intact; CLI read parity remains M31.
...
| Domain and application operations | Complete v0.3 workflows plus M26 terminal-history reads and atomic correction/completion operations; M27 exposes Resolution and Invalid-reason correction; M28-M29 add read-only scorecard and paired-update projections; M30 exposes a confirmed Postmortem completion operation through Dashboard |
...
| Automated tests | Complete v0.1-v0.3 coverage plus M26-M29 correction, scorecard, and update-analytics coverage and M30 temporary-database/Qt coverage for queue membership, cancellation, effective facts, completion, stale context, later Postmortems, and restart |

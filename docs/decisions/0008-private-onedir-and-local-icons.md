...
PyInstaller 6.22.2 is pinned in a separate locked `packaging` dependency group. A checked-in spec produces a windowed `onedir` bundle and explicitly collects the selected resources and notices. The build script keeps generated files and its uv cache under ignored repository paths, relocates a copy of the bundle, and launches that frozen executable against disposable paths. The smoke route has evolved with the source releases: for v0.6 it starts from a real schema-version-13 v0.4 database, migrates through the v0.5 schema-version-15 search and Saved View boundary, exercises complete Binary/Numeric history, search, tag, repair, backup, and independent-read behavior, then loads the packaged v0.6 visual system, local icons, shell modes, every primary screen, both Detail types, guarded shortcuts, responsive sizes, and presentation restart. Logical snapshots of every application and derived SQLite table prove that presentation-only use and restart do not rewrite canonical data.

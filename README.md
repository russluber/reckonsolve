...
v0.7 implementation is in progress through Milestone 49. Schema version 17 preserves every existing Prediction's immutable model/scoring identity and connects trajectory Resolution history to search and Postmortem completion. New Binary creation commits to the trajectory model and requires a permanent exact Forecast Deadline. Numeric creation remains on the released interval model until its later vertical slice; existing Binary and Numeric predictions retain their creation-era behavior.

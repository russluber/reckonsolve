...
v0.7 implementation is in progress through Milestone 51, on schema version 18. New Binary creation uses the trajectory model. New Numeric creation now records five percentiles (q05/q25/q50/q75/q95), an explicit decimal/continuous-style or whole-number constraint, and a permanent exact Forecast Deadline. The desktop and CLI support complete revisions, unchanged Forecast Reviews, anchored Journals, metadata edits, and model-aware browsing. Numeric Detail includes a central CDF preview with no invented tails. Existing Binary and Numeric predictions retain their creation-era editors and behavior.

M51 is an active-forecast slice: new five-quantile Numeric Resolution and visible WIS scorecards arrive in M52. Resolve is disabled for that cohort for now; legacy interval Resolution remains available. Full SQLite backup remains available, while CSV format 3 still rejects databases containing new-model forecasts until M55 supplies the complete export contract.

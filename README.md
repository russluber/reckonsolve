...
The completed binary v0.1 baseline remains intact. Milestone 18 advances the approved staged v0.2 plan: **New Prediction** offers a Binary or Numeric interval choice, with Binary remaining the default quick path. Numeric Detail can append changed intervals, record type-anchored Journal history, resolve an exact realized value—including one outside the interval—or preserve an Invalid decision. Dashboard and Predictions surface both forecast types clearly, and Analytics now adds Numeric containment calibration plus exact-unit error and interval-score summaries without averaging unlike units. Type-aware export and Forecast Reviews remain later v0.2 slices.

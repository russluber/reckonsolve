...
v0.7 implementation is in progress through Milestone 53, on schema version 18. New Binary creation uses the trajectory model. New Numeric creation records five percentiles (q05/q25/q50/q75/q95), an explicit decimal/continuous-style or whole-number constraint, and a permanent exact Forecast Deadline. The desktop and CLI support complete revisions, unchanged Forecast Reviews, anchored Journals, metadata edits, model-aware browsing, and effective-time Resolution. Numeric Detail includes a central CDF preview with no invented tails. Existing Binary and Numeric predictions retain their creation-era editors and behavior.

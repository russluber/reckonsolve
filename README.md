...
v0.7 implementation is in progress through Milestone 47. Schema version 16 preserves every existing Prediction's immutable legacy model/scoring identity. New Binary creation now commits to the trajectory model and requires a permanent exact Forecast Deadline. Numeric creation remains on the released interval model until its later vertical slice; existing Binary and Numeric predictions retain their creation-era behavior.
...
This is a staged development build: new trajectory Binary **resolution/scoring arrives in M48**. Legacy resolution still works. CSV format 3 refuses databases containing new-model records rather than silently omitting their contracts; use **SQLite backup** for complete recovery until the M55 CSV upgrade. Test these changes with `uv run reckonsolve-dev` and `uv run reckonsolve-cli-dev`; keep the released installation for personal forecasting until the full v0.7 release is ready.

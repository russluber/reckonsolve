...
v0.7 implementation is in progress through Milestone 54, on schema version 18. New Binary creation uses the trajectory model. New Numeric creation records five percentiles (q05/q25/q50/q75/q95), an explicit decimal/continuous-style or whole-number constraint, and a permanent exact Forecast Deadline. The desktop and CLI support complete revisions, unchanged Forecast Reviews, anchored Journals, model-aware browsing, and effective-time Resolution; metadata edits and terminal corrections remain desktop workflows. Numeric Detail includes a central CDF preview with no invented tails. Existing Binary and Numeric predictions retain their creation-era editors and behavior.

M54 verifies the four forecast models together across desktop and CLI: mixed retrieval, dynamic Saved Views, tag changes, corrected history, search repair, exact timestamps, stale edits, and locked writes. Numeric CLI detail now identifies its scoring contract; mutation prompts also show its permanent unit, precision, and value constraint. Unsupported model/scoring identities fail clearly rather than disappearing from results. No migration or new CLI command is required. Use `uv run reckonsolve-cli-dev` (or `uv run rscd`) to test the current source alongside `uv run reckonsolve-dev`; an independently installed `rscd` executable can still run an older installed copy.

M52 completes individual five-quantile Numeric Resolution: enter the exact actual value and when it became knowable, or choose **use recording time**. Whole-number Predictions require integral outcomes. Detail and CLI `show` display WIS, the scored revision, and expandable desktop breakdown of median error, both intervals, weighted contributions, and Initial/Final/Delta WIS. A positive Delta means the final forecast scored better within that Prediction, not a universal skill score. **Correct Resolution** can amend actual value or effective time with an explanation; text-only corrections remain lightweight and retain hidden timestamp precision. Original recorded-at and every revision remain unchanged. A time correction can select an earlier revision or make an already-known outcome explicitly unscored. Full SQLite backup remains available, while CSV format 3 still rejects new-model databases until M55.

M53 adds **Five-quantile Numeric** feedback to Analytics. Continuous-style forecasts compare the five stated percentiles with observed frequencies; whole-number forecasts show separate strictly-below and at-or-below frequencies so ties are not lost. Both groups show sample size, 50%/90% below-inside-above outcomes, median balance, and 95% Wilson uncertainty ranges. Small samples remain visible but uncertain. A whole-number interval containing more than its nominal percentage is not automatically miscalibrated. Updated forecasts are counted as better/equal/worse relative to their initial WIS, with unrevised forecasts separate; unrelated questions never receive a pooled raw WIS or WIS-improvement average. Forecast type, tag, and exact-unit filters apply throughout. Legacy interval-v1 analytics stay separately labeled and unchanged.

For new Binary predictions, the desktop accepts a date/time and explicit UTC offset (check daylight saving time for the chosen date). CLI `create binary` accepts an ISO date/time such as `2099-10-01T18:00:00-07:00`; enter `?` at that prompt for optional forecasting guidance. Revisions and Reviews stop at the exact Deadline, while Locked predictions still accept Journals. The deadline is visible but cannot be edited. Expected Resolution remains a separate optional date.

New Binary Resolution records when the outcome first became fixed and knowable separately from when you record it. Choose **use recording time** only if that is now; otherwise enter the earlier source time and its UTC offset. The desktop normally displays hours and minutes, with **Show seconds** for finer entry. CLI `resolve ID` accepts `now` or an offset-bearing ISO timestamp. Neither interface accepts an effective time later than recording time.

Resolved trajectory Detail and CLI `show ID` display **Trajectory Brier**, with Initial/Final Brier, hold-initial comparison, Updating Gain, and Active Forecast Fraction as supporting diagnostics. Early resolution gives the remaining planned window neutral loss, not an invented forecast. Revisions at or after the effective scoring cutoff remain in history with an exclusion label. Outcomes already fixed at or before the first forecast are explicitly unscored. **Correct Resolution** can amend the effective time or outcome with an explanation; original facts, recorded-at, and all revisions remain intact. A notes-only correction preserves the exact saved time, including precision hidden by the ordinary display.

This is a staged development build. Trajectory Binary aggregates are now separate from legacy Binary analytics: each eligible Prediction receives one equal vote in mean Trajectory Brier, while resolution timing, active-window share, updating comparisons, and final-probability calibration remain clearly labeled diagnostics. Legacy resolution/scoring remains unchanged. CSV format 3 refuses databases containing new-model records rather than silently omitting their contracts; use **SQLite backup** for complete recovery until the M55 CSV upgrade. Test with `uv run reckonsolve-dev` and `uv run reckonsolve-cli-dev`; keep the released installation for personal forecasting until the full v0.7 release is ready.

The current application creates a Binary prediction from a question, any whole-number probability from 0% through 100%, and its exact Deadline. A collapsed **More details** section accepts an optional initial rationale, Background, Resolution Criteria, Expected Resolution, and tags; the complete initial state, contract, and first forecast are saved atomically. Prediction Detail displays current forecast and metadata, supports safe metadata editing, and appends probability revisions with an optional rationale without rewriting earlier forecasts. Legacy Binary records retain their editable optional date-only deadline.
...
Question, Resolution Criteria, and legacy Forecast Deadline edits require tailored confirmation and remain visible in a collapsed Definition history. Legacy date-only Deadlines include the selected day; new Binary exact Deadlines reject revisions and Reviews at or after that instant. Locked predictions still accept Journals. Resolved and Invalid predictions reject new entries and Reviews, but existing Journal entries can still receive audited corrections. Unchanged probabilities are not recorded as revisions: use a Forecast Review for deliberate reconsideration and retention, or a Journal entry for ordinary evidence and reasoning. All of this history persists across restarts.

Open and Locked predictions can be resolved Yes or No or marked Invalid. Legacy Resolution captures the final scoring revision plus optional factual notes and a reflective Postmortem; trajectory Binary scoring instead derives eligible revisions from the immutable history, Deadline, and latest effective outcome time. Invalidation preserves an optional reason and excludes the prediction from scoring. Terminal state and original records remain immutable; confirmed corrections append history and score-affecting changes require an explanation. An untouched Open duplicate or test record can instead be permanently deleted after confirmation. Once a prediction is Locked, revised, edited, journaled, Resolved, or Invalid, its normal Delete action is unavailable and meaningful nonterminal history is directed toward Invalid.
...
The Analytics screen keeps scoring cohorts explicit. Each eligible trajectory Binary Prediction contributes one equally weighted Trajectory Brier regardless of window length; early-versus-deadline counts and mean Active Forecast Fraction explain timing, while Initial/Final Brier, hold-initial comparison, and Updating Gain are descriptive hindsight diagnostics. Its reliability view uses one final standing probability before the effective cutoff and is deliberately labeled final-probability calibration—not trajectory calibration; neutral truncation contributes no invented forecast. Legacy Binary views separately retain captured-final mean Brier, ten-bin reliability, cumulative mean Brier, and initial/final feedback. Numeric interval views remain separate and show ten-bin containment calibration plus exact-unit magnitude scores. Open, Locked, Invalid, and outcome-already-fixed trajectory records contribute no score or calibration observation. Forecast type, tag, and unit filters recompute every visible value over one consistent subset.
...
The desktop uses a deliberately small, offline subset of Lucide icons while retaining action text or accessible names. Its centralized visual system follows the active native Qt palette and font rather than imposing a custom theme. The sidebar can collapse to complete icon-only navigation, and safe geometry plus sidebar mode are stored in `presentation.ini` outside SQLite. Development runs identify themselves as **Reckonsolve Dev** and use a separate database and presentation settings from the stable channel. A repeatable private PyInstaller `onedir` build validates the complete v0.5 data boundary together with v0.6 style resources, shell modes, primary screens, both forecast types, keyboard navigation, responsive sizes, search, backup, and restart. Original application-icon artwork still awaits user direction; a normal installer, signing, and public binary distribution are deliberately deferred.

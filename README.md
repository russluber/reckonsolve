...
The completed binary v0.1 baseline remains intact. Milestone 16 advances the approved staged v0.2 plan: **New Prediction** offers a Binary or Numeric interval choice, with Binary remaining the default quick path. Numeric Detail can append changed intervals, record type-anchored Journal history, and show the causal timeline plus interval-band/median chart. Open or Locked Numeric Predictions can now resolve to an exact realized value—including one outside the forecast interval—or be marked Invalid; resolution captures exactly one final interval revision for later scoring. Untouched Open Numeric test records may be deleted after confirmation, while meaningful and terminal history is preserved. Numeric archive, Dashboard, analytics, export, and Forecast Reviews remain later v0.2 slices.

...
The completed binary v0.1 baseline remains intact. Milestone 14 begins the approved staged v0.2 plan: **New Prediction** now offers a Binary or Numeric interval choice, with Binary remaining the default quick path. A Numeric Prediction records a Question, unit, fixed decimal precision, inclusive lower/median/upper interval, and confidence; optional initial rationale, metadata, dates, and tags are saved atomically with its first interval. Numeric Prediction Detail displays that exact current interval and metadata after restart. Numeric revisions, Journal entries, lifecycle resolution, browsing, analytics, and Forecast Reviews remain planned later v0.2 slices.

...
The completed binary v0.1 baseline remains intact. Milestone 15 advances the approved staged v0.2 plan: **New Prediction** offers a Binary or Numeric interval choice, with Binary remaining the default quick path. A Numeric Prediction records a Question, unit, fixed decimal precision, inclusive lower/median/upper interval, and confidence; optional initial rationale, metadata, dates, and tags are saved atomically with its first interval. Numeric Detail can append a changed interval with an optional rationale, record and transparently correct Journal entries anchored to the exact interval in effect, and show the resulting causal timeline plus an interval-band/median history chart. Numeric resolution, archive, Dashboard, analytics, and Forecast Reviews remain later v0.2 slices.

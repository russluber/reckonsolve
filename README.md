...
v0.7 implementation is in progress through Milestone 50. Schema version 18 adds the internal five-quantile Numeric storage and exact WIS-scoring foundation while preserving every existing Prediction and history anchor. New Binary creation commits to the trajectory model and requires a permanent exact Forecast Deadline. Numeric creation still uses the released interval model: the five-quantile GUI/CLI switch belongs to M51, not this foundation milestone. Existing Binary and Numeric predictions retain their creation-era behavior.
